    ...
    <nav className="bg-gray-800 text-white p-4 top-0 w-full shadow-md">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex space-x-8 items-center text-lg font-semibold">
          ...
          {isAuthenticated && <Link to="/profile" className="hover:text-gray-300">Profile</Link>}
      ...
      {menuOpen && (
        <div className="md:hidden flex flex-col space-y-3 mt-4 px-4 text-lg font-semibold">
          <Link to="/search" onClick={() => setMenuOpen(false)} className="hover:text-gray-300">Search</Link>
          ...
          <Link to="/info" onClick={() => setMenuOpen(false)} className="hover:text-gray-300">Info</Link>
          {isAuthenticated ? (
            <button
              onClick={() => {
                logout();
              ...
              className="bg-[#2ac9ff] px-4 py-2 rounded-lg"
            >
              Logout
            </button>
          ) : (
